Scriptname vSS_Registry Hidden
...
Function SendRegEvent(String asPath) Global
	Int iHandle = ModEvent.Create("vSS_RegUpdate")
	If iHandle
		ModEvent.PushString(iHandle,asPath)
		ModEvent.Send(iHandle)
	Else
		;Debug.Trace("vSS/Reg: Error sending RegUpdate event!",1)
	EndIf
EndFunction
...
Function SyncReg() Global
	Int jRegData = JDB.solveObj(".vSS.Registry")
	If !jRegData
		jRegData = JMap.Object()
		JDB.solveObjSetter(".vSS.Registry",jRegData,True)
	EndIf
	Int jRegFileData = JValue.ReadFromFile(SuperStash.userDirectory() + "Config/vSS_Registry.json")
	Int DataSerial = JMap.getInt(jRegData,"DataSerial")
	Int DataFileSerial = JMap.getInt(jRegFileData,"DataSerial")
	;Debug.Trace("vSS/Reg: SyncReg called! Our DataSerial is " + DataSerial + ", file DataSerial is " + DataFileSerial)
	If DataSerial > DataFileSerial
		;Debug.Trace("vSS/Reg: Our data is newer than the saved file, overwriting it!")
		JValue.WriteToFile(jRegData,SuperStash.userDirectory() + "Config/vSS_Registry.json")
	ElseIf DataSerial < DataFileSerial
		;Debug.Trace("vSS/Reg: Our data is older than the saved file, loading it!")
		JValue.Clear(jRegData)
		jRegData = JValue.ReadFromFile(SuperStash.userDirectory() + "Config/vSS_Registry.json")
		JDB.solveObjSetter(".vSS.Registry",jRegData)
	Else
		;Already synced. Sunc?
	EndIf
EndFunction
...
Function LoadReg() Global
	;Debug.Trace("vSS/Reg: LoadReg called!")
	Int jRegData = JDB.solveObj(".vSS.Registry")
	jRegData = JValue.ReadFromFile(SuperStash.userDirectory() + "Config/vSS_Registry.json")
EndFunction
...
Int Function CreateRegDataIfMissing() Global
	...
	If jRegData
		...
		Return jRegData
	EndIf
	;Debug.Trace("vSS/Reg: First RegData access, creating JDB key!")
	Int _jvSS = JDB.solveObj(".vSS")
	jRegData = JValue.ReadFromFile(SuperStash.userDirectory() + "Config/vSS_Registry.json")	
	If jRegData
		;Debug.Trace("vSS/Reg: Loaded Reg file!")
	Else
		;Debug.Trace("vSS/Reg: No Reg file found, creating new RegData data!")
		jRegData = JMap.Object()
		JMap.setInt(jRegData,"DataSerial",0)
	EndIf
	JMap.setObj(_jvSS,"Registry",jRegData)
	Return jRegData
EndFunction
...
String Function GetRegStr(String asPath) Global
	Return JDB.solveStr(".vSS.Registry." + asPath)
EndFunction
...
Bool Function GetRegBool(String asPath) Global
	Return JDB.solveInt(".vSS.Registry." + asPath) as Bool
EndFunction
...
Int Function GetRegInt(String asPath) Global
	Return JDB.solveInt(".vSS.Registry." + asPath)
EndFunction
...
Float Function GetRegFlt(String asPath) Global
	Return JDB.solveFlt(".vSS.Registry." + asPath)
EndFunction
...
Form Function GetRegForm(String asPath) Global
	Return JDB.solveForm(".vSS.Registry." + asPath)
EndFunction
...
Int Function GetRegObj(String asPath) Global
	Return JDB.solveObj(".vSS.Registry." + asPath)
EndFunction
